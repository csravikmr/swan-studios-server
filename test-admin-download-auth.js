...
async function testAdminDownload() {
    try {
        ...
        const loginUrl = 'http://localhost:9000/admin/auth';

        // Try to authenticate using the provided credentials
        const credentials = {
            email: 'csravikmr@gmail.com',
            password: 'goldeneye'
        };
        ...
        // If login was successful, we should have session cookies
        if (loginResponse.status !== 200 && loginResponse.status !== 201) {
            const errorText = await loginResponse.text();
            console.error(`Login failed: ${errorText}`);

            // Try alternative authentication method - maybe using API key
            console.log('Trying alternative authentication with API key...');

            // Check if there's an admin API key in environment
            ...
            console.log(`Making request to: ${url} with API key`);
        ...
        // Step 2: Use the session cookie to download the invoice
        console.log('Step 2: Downloading invoice with session cookie...');
        const downloadUrl = `http://localhost:9000/admin/invoices/${invoiceId}/download`;
        ...
        if (downloadResponse.status === 200) {
            ...
            // Save the file for inspection
            const outputPath = path.join(__dirname, 'test-admin-download-auth.pdf');
            fs.writeFileSync(outputPath, pdfBuffer);
            console.log(`PDF saved to: ${outputPath}`);
            ...
            console.error(`Error response: ${errorText}`);
            return { success: false, status: downloadResponse.status, error: errorText };
        }
    } catch (error) {
